...
model.compile(optim, weighted_jaccard_loss, metrics)
...
train_dataloader = Dataloder(train_dataset, batch_size=BATCH_SIZE, shuffle=True, length=train_len // batch_size * 8, train_len=train_len)
valid_dataloader = Dataloder(valid_dataset, batch_size=BATCH_SIZE, shuffle=True, length=valid_len // batch_size * 8, validation=True, validation_len=valid_len)

#model desc
model_num = 1
loss = 'sum_jaccard_loss'
image_size = str(img_size[0])
image_mode = 'cropped_w_edges'
add_info = f''
